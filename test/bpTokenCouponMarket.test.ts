...
describe("Boardroom", () => {
    ...
    beforeEach("deploy contracts", async () => {
        ...
        bpdot = await bpDOT.connect(operator).deploy();
        ...
        subboard = await bpTokenBoardroom.connect(operator).deploy();
        await subboard.connect(operator).initialize(boardroom.address);
    });
    ...
    describe("#withdraw", () => {
        ...
        it("should fail when non-director tries to withdraw", async () => {
            await expect(boardroom.connect(abuser).withdraw(ZERO)).to.revertedWith("Boardroom: The _director does not exist");
        });
    ...
    describe("#allocateSeignioragePegToken", () => {
        beforeEach("initialize and stake", async () => {
            await subboard.connect(operator).addPegToken(bpdot.address);
            await boardroom.connect(operator).addPegToken(bpdot.address, subboard.address);
            // await boardroom.connect(operator).setBpTokenBoardroom(bpdot.address, subboard.address);
            await Promise.all([
                share.connect(operator).distributeReward(rewardPool.address),
                share.connect(rewardPool).transfer(whale.address, STAKE_AMOUNT),
                share.connect(whale).approve(boardroom.address, STAKE_AMOUNT),
        ...
        it("should allocate seigniorage of bpDOT to stakers", async () => {
            await bpdot.connect(operator).mint(operator.address, SEIGNIORAGE_AMOUNT);
            await bpdot.connect(operator).approve(boardroom.address, SEIGNIORAGE_AMOUNT);

            await expect(boardroom.connect(operator).allocateSeignioragePegToken(bpdot.address, SEIGNIORAGE_AMOUNT))
              .to.emit(subboard, "PegTokenRewardAdded")
              .withArgs(bpdot.address, boardroom.address, SEIGNIORAGE_AMOUNT);

            expect(await boardroom.earnedPegToken(bpdot.address, whale.address)).to.eq(SEIGNIORAGE_AMOUNT);
        });

        it("should fail when user tries to allocate with zero amount", async () => {
            await expect(boardroom.connect(operator).allocateSeignioragePegToken(bpdot.address, ZERO)).to.revertedWith("bpTokenBoardroom: Cannot allocate 0");
        });

        it("should fail when non-operator tries to allocate seigniorage", async () => {
            await expect(boardroom.connect(abuser).allocateSeignioragePegToken(bpdot.address, ZERO)).to.revertedWith("Boardroom: caller is not the operator");
        });

        it("should claim dividends", async () => {
            await bpdot.connect(operator).mint(operator.address, SEIGNIORAGE_AMOUNT);
            await bpdot.connect(operator).approve(boardroom.address, SEIGNIORAGE_AMOUNT);
            await boardroom.connect(operator).allocateSeignioragePegToken(bpdot.address, SEIGNIORAGE_AMOUNT);

            await expect(boardroom.connect(whale).claimReward()).to.emit(subboard, "PegTokenRewardPaid").withArgs(bpdot.address, whale.address, SEIGNIORAGE_AMOUNT);
            expect(await boardroom.balanceOf(whale.address)).to.eq(STAKE_AMOUNT);
            expect(await bpdot.balanceOf(whale.address)).to.eq(SEIGNIORAGE_AMOUNT);
        });
    });
});
